 ...
 *
 * Description
 * Create a new game called "BUILD", a version of the game "High-Card". Use
 * multithreading to create a timer for the game that runs while the game is
 * played.
 * 
 */
...
public class Assig6
{
   ...
   private static class Model
   {
      ...
      static Card[][] playerWinnings = new Card[NUM_PLAYERS]
         [NUM_CARDS_PER_HAND * 2];
      
      //Contains the point values for each of the cards, in the same order as
      //the valid cards are held in the validValues array of the Card class.
      ...
      public Model(Controller controller, View view)
      {
         ...
         this.view = view;
         this.highCardGame = new CardGameFramework(this.numPacksPerDeck,
            this.numJokersPerPack, this.numUnusedCardsPerPack,
            this.unusedCardsPerPack, Assig6.NUM_PLAYERS,
            Assig6.NUM_CARDS_PER_HAND);
      } // end Model()
      ...
      private static char[] getValidValues(Card card)
      {
         ...
         {
            validValues[0] = cardValues.charAt(cardValues
               .indexOf(card.getValue()) - 1);
            validValues[1] = cardValues.charAt(cardValues
               .indexOf(card.getValue()) + 1);
         }
      ...
      private static void playComputerCard()
      {
         ...
         if (computerPlay[0] != -1)
         {
            ...
            view.setStatusText("Computer played " + computerCard + " on Stack "
               + (computerStack+1) + ".");
         ...
         else
         {
            ...
            //The computer does not have a card at this point. Indicate a
            //skipped turn by returning -1.
         ...
         {
            view.setStatusText(view.getStatusText() + " A new card has been "
               + "placed on each stack.");
      ...
      } // end playComputerCard()
      
      //returns true if the card was placed.
      public static boolean placeCard(int stack)
      {
         Card playerCard = highCardGame.getHand(HUMAN_PLAYER)
            .inspectCard(selectedCard);
         char[] validValues = getValidValues(cardStacks[stack]
            .inspectTopCard());
         ...
         for (char value : validValues)
         {
            if (value == playerCard.getValue())
            {
               ...
               view.drawHand(HUMAN_PLAYER, highCardGame
                  .getHand(HUMAN_PLAYER), true);
         ...
         view.setStatusText("That card cannot go there... Select a different"
            + "card.");
         return false;
      } // end placeCard
      ...
      private static void skipPlayerTurn()
      {
         ...
         if (play[0] != -1)
         {
            view.setStatusText("You have a card that can be placed on Stack "
               + (play[0] + 1) + ".");
            return;
         }
      ...
      private static void initGame()
      {
         ...
         view.setStatusText("Click on a card in your hand, then click on the"
            + "card stack you would like to play it on.");
       ...
       * Out: An integer representing the position in the computer's hand of the
       *      card it chooses to play.
       * This function makes this game very hard to win. The computer knows
       * which card the human chooses to play. It chooses which card to play
       * based on this.
       */
      ...
      private static void gameOver()
      {
         ...
         if (winner == HUMAN_PLAYER)
            view.setStatusText(view.getStatusText() + "You beat the computer! "
               + "Good job! Click here to play again!");
         else if (winner == 0)
            view.setStatusText(view.getStatusText() + "The computer beat you..."
   ...
   private static class View
   {
      ...
      static JButton skipTurnButton = new JButton(SKIP_TURN);
      ...
      private static void addButtonListener()
      {
         ...
         timerStartBttn.addActionListener(model.controller);
         timerStopBttn.addActionListener(model.controller);
      } // end addButtonListener()
      
      private static void removeButtonListener()
      {
         skipTurnButton.removeActionListener(model.controller);
         timerStartBttn.removeActionListener(model.controller);
         timerStopBttn.removeActionListener(model.controller);
      ...
      public View(String title, int numPlayers, int numCardsPerHand)
      {
         ...
         for (int i = 0; i < playedCardLabels.length; i++)
         {
            ...
            playedCardLabels[i].addMouseListener(Assig6.controller);
         ...
         timerStartBttn.setBorderPainted(true);
         timerStartBttn.setOpaque(true);
         cardTable.pnlTimerSubBttn.add(timerStartBttn);
         timerStopBttn.setBackground(Color.RED);
         timerStopBttn.setBorderPainted(true);
         timerStopBttn.setOpaque(true);
         cardTable.pnlTimerSubBttn.add(timerStopBttn);
      ...
      public void drawHand(int player, Hand hand, boolean showCards)
      {
         ...
         for (int i = 0; i < hand.getNumCards(); i++)
         {
            ...
            if(showCards)
            {
               this.playerHands[player][i].setIcon(GUICard.getIcon(hand
                  .inspectCard(i)));
      ...
      public static class GUICard
      {
         ...
         /* In: Nothing
          * Out: Nothing
          * Loads all of the card icons from the images directory. If the images
         ...
         /* In: A card object
          * Out: An integer representing the row in iconCards that contains that
          * value.
          */
         ...
         /* In: A card object
          * Out: An integer representing the column in the iconCards array that
          * contains that suit.
          */
      ...
      /* CardTable implements JFRame in order to draw a GUI card table. It
       * contains three main panels, one for the human's hand, one for the
       * computer's, and one for the play area.
       */
      class CardTable extends JFrame
      {
         ...
         public JPanel pnlPlayArea, pnlPlayedCards, pnlPlayerText,
            pnlStatusText, pnlNoPlays, pnlPlayedCardArea, pnlTimer, 
          ...
          *     [3] An integer value representing the number of players playing
          *         on the table
          * Out: Nothing.
          */
         public CardTable(String title, int numCardsPerHand, int numPlayers)
         {
            ...
            if (numCardsPerHand < 0 || numCardsPerHand > CardTable
               .MAX_CARDS_PER_HAND)
            {
            ...
            //The card table will use a BorderLayout style. This allows each
            //panel to have a different height, which allows for a larger play
            //area and smaller hand areas.
            BorderLayout layout = new BorderLayout();
            this.setLayout(layout);

            //Both the computer and human hand panels will use the flow layout.
            ...
            this.handPanels[0].setPreferredSize(new Dimension((int) this
               .getMinimumSize().getWidth() - 50, 105));
            
            //Use a JScrollPane in case the cards per hand is greater than can
            //be displayed in the panel without a scroll bar.
            JScrollPane scrollComputerHand = new JScrollPane(this
               .handPanels[0]);
            ...
            //The play area will use a grid layout, so that the played cards,
            //labels, and status text can be displayed in neat columns.
            ...
            //
            //   Creates and formats JPanel for timer and buttons.
            //
            ...
            this.handPanels[1].setPreferredSize(new Dimension((int) this
               .getMinimumSize().getWidth() - 50, 105));
   ...
   private static class Controller implements MouseListener, ActionListener
   {
      ...
      public void mouseEntered(MouseEvent e)
      {
         ...
         if (source == view.playedCardLabels[0] || source == view
            .playedCardLabels[1])
         {
            if (stackClickable)
      ...
      public void mouseClicked(MouseEvent e)
      {
         ...
         for (int playerHand = 0; playerHand < view.playerHands.length;
            playerHand++)
         {
            for (int card = 0; card < model.highCardGame.getHand(playerHand)
               .getNumCards(); card++)
            {
               if (view.playerHands[playerHand][card].getIcon() == View.GUICard
                  .getBackCardIcon())
                  continue;
               ...
               if (view.playerHands[playerHand][card] == source)
               {
                  ...
                  stackClickable = true;
                  view.setStatusText("Click on the stack you would like to "
                     + "place the selected card on.");
                  view.highlightLabel(source);
                  return;
               }
            } // end for loop card
         } // end for loop playerHand
         
         //If this function has gone this far, determine if the label being
         //clicked is one of the stacks, as long as stackClickable is true.
         if (stackClickable)
         {
      ...
      public void actionPerformed(ActionEvent e)
      {
         ...
         if (source.getActionCommand() == Assig6.SKIP_TURN)
         {
            model.skipPlayerTurn();
         }

         else if (source.getActionCommand() == Assig6.START_TIMER)
         {
         ...
         else
         {
            ...
            finally
            {
               ...
               {
                  view.timerStartBttn.setText(Assig6.START_TIMER);
               }
            } // end finally
         }
      } // end actionPerformed()
...
/* Class Deck represents a deck of cards consisting of a variable number of 52
 * card packs. It contains a master pack, which the deck is built off of. It
 * also contains member functions that would be expected of a deck of cards,
 * providing functionality like shuffling and dealing.
 */
class Deck
{
   ...
   //The cards in the object's deck. Not static, as each deck object can have
   //different cards.
   private Card[] cards;
   
   //The position of the card on the top of the deck.
   private int topCard;
   
   //The deck can consist of multiple packs of cards.
    ...
    * Out: A boolean value indicating whether the card was able to be added to
    *      the deck
    */
   ...
   /* In: Nothing
    * Out: An integer indicating the number of cards in the deck.
    */
    ...
    * Out: A boolean value indicating if the card was able to be removed from
    *      the deck.
    */
...
/* Class Card represents a typical card that would be found in a deck of playing
 * cards. It has private members to hold the value and suit of the card. It also
...
class Card implements Comparable
{
   ...
   {
      //Swapped will change to true if any swapping occurs in the loop below.
      ...
      do
      {
         ...
         {
            //If an element is larger than he one after it,
    ...
    *              card. This will create a card of the specified value and
    *              suit.
    */
...
/* Class Hand represents a hand of cards. This is much like a collection of
 * cards, but provides methods or interaction with the cards that are specific
 * to a "hand," rather than a collection.
 */
class Hand
{
   ...
   /* Card playCard()
    * In: Nothing
    * Out: A Card object with the same values as the card on the top of the
    *      hand.
